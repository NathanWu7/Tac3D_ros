...
class Sensor_ros1:
    def __init__(self, recvCallback = None, port = 9988, maxQSize = 5, callbackParam = None):
        ...
        self.frame_pub = rospy.Publisher('tactile_frame',Frame, queue_size=5)
    ...
    def _recvCallback_UDP(self, data, addr):
        ...
        if currBuffer[2] == currBuffer[1]+1:
            ...
            self.frame_msg.SN = frame['SN']
            self.frame_msg.index = frame['index']
            self.frame_msg.sendTimestamp = float(frame['sendTimestamp'])
            self.frame_msg.recvTimestamp = float(frame['recvTimestamp'])
            self.frame_msg.Positions = frame['3D_Positions'].flatten().tolist()
            self.frame_msg.Displacements = frame['3D_Displacements'].flatten().tolist()
            self.frame_msg.Forces = frame['3D_Forces'].flatten().tolist()
            self.frame_msg.ResultantForce = frame['3D_ResultantForce'].flatten().tolist()
            self.frame_msg.ResultantMoment = frame['3D_ResultantMoment'].flatten().tolist()
...
if __name__ == '__main__':
    ...
    tac3d = Sensor_ros1(recvCallback=Tac3DRecvCallback, port=9988)
